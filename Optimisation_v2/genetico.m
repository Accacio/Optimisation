...
while Iter_num < n
    ...
    fit = fit/R;                     % fator de redução de população
    
    F = 500;
    if max(fit) > F                        % Normalização
       fit = fit*F/max(fit);        
    end
    ...
    while c < length(g1x)+1
        ...
        if dec_fit(c) == inf                  % saturação para dec_fit (as vezes da inf)
            dec_fit(c) = 0;
        elseif isnan(c)
            dec_fit(c) = 0;
        end      
    ...
    while c < sz_shuf(2)+1   % numero de colunas de g_inter_shuffled
